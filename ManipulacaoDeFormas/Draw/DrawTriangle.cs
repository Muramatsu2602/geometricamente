...
namespace Draw
{
	...
	public class DrawTriangle : DrawObject
	{
        ...
        public DrawTriangle(float x, float y, float width, float height)
        {
            rectangle.X = x;
            rectangle.Y = y;
...
/*		[CLSCompliant(false)]
		public bool FillFromSVG(SvgRect svg)
		{
			...
			{
				ErrH.Log("DrawTriangle", "SvgRect invalid", ex.ToString(), ErrH._LogPriority.Info);
				return false;
			}
		}*/
		...
		public override void Draw(Graphics g)
		{
			try
			{
				RectangleF r = DrawTriangle.GetNormalizedRectangle(RectangleF);
				if (Fill != Color.Empty)
				{
				...
				//g.DrawTriangle(pen,r.X,r.Y,r.Width,r.Height);

				pen.Dispose();
			} 
			catch (Exception ex)
			{
				ErrH.Log("DrawTriangle", "Draw", ex.ToString(), ErrH._LogPriority.Info);
			}
		}
        ...
        public override bool IntersectsWith(RectangleF rectangle)
        {
			...
			{
				ErrH.Log("DrawTriangle", "Intersect", ex.ToString(), ErrH._LogPriority.Info);
				return false;
			}
        }
        ...
        {
            rectangle = DrawTriangle.GetNormalizedRectangle(rectangle);
        }
		...
		public static DrawTriangle Create(SvgRect svg)
		{
			try
			{
				DrawTriangle dobj = new DrawTriangle(DrawObject.ParseSize(svg.X,DrawObject.Dpi.X),
			...
			{
				ErrH.Log("DrawTriangle", "CreateRectangle:", ex.ToString(), ErrH._LogPriority.Info);
				return null;
			}
		}
