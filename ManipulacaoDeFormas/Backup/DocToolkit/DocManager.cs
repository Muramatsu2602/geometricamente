...
namespace DocToolkit
{
    ...
    public class DocManager
    {
        ...
        public DocManager(DocManagerData data)
        {
			...
			if ( config != null )
				fileDlgInitDir = config.GetSetting("FileDir/Path",Application.StartupPath);
...
//                fileDlgInitDir = new FileInfo(newFileName).DirectoryName;
			...
			fileDlgInitDir = new FileInfo(newFileName).DirectoryName;
			Environment.CurrentDirectory = (fileDlgInitDir);
...
//                           newFileName, FileMode.Open, FileAccess.Read) ) 
                {
					...
					XmlTextReader reader = new XmlTextReader(newFileName);
					//IFormatter formatter = new BinaryFormatter();

                    if ( LoadEvent != null )        // if caller subscribed to this event
                    {
                        SerializationEventArgs args = new SerializationEventArgs(reader, newFileName);
        ...
        public bool SaveDocument(SaveType type) 
        {
			...
			try 
			{
				StreamWriter writer = File.CreateText(newFileName);
				if ( SaveEvent != null )        // if caller subscribed to this event
				{
					SerializationEventArgs args = new SerializationEventArgs(writer,newFileName);
					SaveEvent(this, args);
					if ( args.Error )
						return false;
				}
        ...
        {
			Config config = Config.Instance();
			config.PutSetting("FileDir/Path",fileDlgInitDir);
		}
    ...
    public class SerializationEventArgs : System.EventArgs
    {
		...
		public SerializationEventArgs(StreamWriter writer, string fileName)
		{
			_Writer = writer;
            this.fileName = fileName;
            errorFlag = false;
        }
		public SerializationEventArgs(XmlTextReader _xmlReader, string fileName)
		{
